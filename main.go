...
func main() {
	...
	init.EtcdCert(k8sMasterHost)

	log.Println("依据CA机构证书生成kube-apiserver证书")
	init.KubeApiserverCert(k8sMasterHost)
	...
	init.KubeControllerManagerCert(k8sMasterHost)

	log.Println("依据CA机构证书生成kube-scheduler证书")
	init.KubeSchedulerCert(k8sMasterHost)

	log.Println("=+=+=+=+=+=+=+==+=+=+=+=+=+=+初始化各组件配置文件=+=+=+=+=+=+=+=+=+=+=+=+=+")
	log.Println("初始化etcd配置文件")
